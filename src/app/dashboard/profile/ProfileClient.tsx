...
function DynamicProfileContent({ userPromise, mounted }: { userPromise: Promise<any>, mounted: boolean }) {
  ...
  const [year, setYear] = useState(initialUser?.academicYear || "1ST");
  const [selectedTools, setSelectedTools] = useState<string[]>(initialUser?.tools || []);
  const [xpLevel, setXpLevel] = useState<XPLevel>(initialUser?.xpLevel || "BEGINNER");
  ...
  return (
    <div className="bg-[#131314] text-[#E5E2E3] min-h-screen relative overflow-hidden">
      ...
      <div className="max-w-[1440px] mx-auto w-full px-6 md:px-8 xl:px-16 pt-[60px] md:pt-[100px] pb-16">
        ...
        {/* ── MAIN CONTENT GRID ── */}
        <React.Suspense fallback={<ProfileDynamicSkeleton />}>
          <DynamicProfileContent userPromise={userPromise} mounted={mounted} />
        </React.Suspense>
      </div>
    </div>
  );
